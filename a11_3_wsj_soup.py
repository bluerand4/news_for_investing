...
payload = ""
headers = {
    "cookie": "AWSALB=8QjsxW2OK9Y9nrmGzZQcF7NolNzQLHdPVDXYsYVyQCICrhzQud%2FgYh7ZB0UU7siRrGp1NujS4NU5Wi3L7YyRaPuNjAYLqk8W973MPSEA85kbXaftD0CTG7oTckIH; AWSALBCORS=8QjsxW2OK9Y9nrmGzZQcF7NolNzQLHdPVDXYsYVyQCICrhzQud%2FgYh7ZB0UU7siRrGp1NujS4NU5Wi3L7YyRaPuNjAYLqk8W973MPSEA85kbXaftD0CTG7oTckIH; FixedFacetDefaults=20752010; BrowserSession=0%7C0%7C%7Caff55d41-585b-4f2f-af1b-96f6a9d3f47e%7CFalse%7C0%7C0%7C133459849910118755%7C133460029910118755%7C%7C%7C%7C1%7C0%7C1%7C00000000-0000-0000-0000-000000000000%7CFalse%7C%7C%7C; AnonymousUserId=31e43930-ebcc-4366-b82d-0806f0f0b6dc; JSMRI=eyJjIjoxLCJkY3MiOiIyMDIzLTEyLTAyVDE1OjAzOjExLjAxMTg3NTUrMDA6MDAiLCJkbHMiOm51bGx9",
...
text
#%%
div_content = soup.find_all('li', class_="cf block lister__item lister__item--article")
div_content
...
#%%
list1=[]
for item in biolist[:10]:
    define='summarize with this format. [company name] has a good(or bad) news. Their drug [drug_name] is... \n However, if no specific company name is present, just summarize. If there are difficult medical terms, define it'
    print('start ')
...
payload = ""
headers = {
    "cookie": "DJSESSION=country%253Dkr%257C%257Ccontinent%253Das%257C%257Cregion%253D11; wsjregion=na%252Cus; gdprApplies=false; ccpaApplies=false; vcdpaApplies=false; regulationApplies=gdpr%253Afalse%252Ccpra%253Afalse%252Cvcdpa%253Afalse; ab_uuid=859ac3e1-b43f-42f0-bda3-fc8b6dfca534; usr_prof_v2=eyJpYyI6MX0%253D",
    "User-Agent": "insomnia/8.4.2"
}
...
div_content = soup.find_all('div', class_='style--grid--SxS2So51 style--full-width--105xgnPD style--padding-bottom--2Q75WlSu styles--padding-bottom--DQ6elYpe style--margin-bottom--3uv3Nt9X styles--margin-bottom-large--wa4U95NA style--border-bottom-dark--3GK54cU_')
div_content
main_headline=div_content[0].text
wsj_list.append(main_headline)
#%%
div_content = soup.find_all('article', class_="WSJTheme--story--XB4V2mLz WSJTheme--story-padding--1gRL3tuf WSJTheme--media-margin-bottom--1bIRFuDR WSJTheme--no-overflow--2dsA4DmQ WSJTheme--border-bottom--s4hYCt0s")
main_headline=div_content[0].text
wsj_list.append(main_headline)
#%%
div_content = soup.find_all('div', class_="style--column--1p190TxH style--column-top--3Nm75EtS style--column-8--2_beVGlu style--border-left--1FbHaAV_")
semi_headline=div_content[0].text
wsj_list.append(semi_headline)
#%%
div_content = soup.find_all('div', class_="style--column--1p190TxH style--column-top--3Nm75EtS style--column-4--2Ng-GQLy style--no-left-padding--39_DNs5J")
semi_headline=div_content[0].text
wsj_list.append(semi_headline)
#%%
div_content = soup.find_all('div', class_="style--column--1p190TxH style--column-top--3Nm75EtS style--column-4--2Ng-GQLy style--no-right-padding--1-C2KF9M")
semi_headline=div_content[0].text
wsj_list.append(semi_headline)
#%%
#class="style--column--1p190TxH style--column-top--3Nm75EtS style--column-4--2Ng-GQLy style--no-left-padding--39_DNs5J "
sub_right = soup.find_all('article', class_="WSJTheme--story--XB4V2mLz WSJTheme--padding-top-large--2v7uyj-o styles--padding-top-large--3rrHKJPO WSJTheme--margin-top-large--280saxOy styles--margin-top-large--2EMcMHz9 WSJTheme--border-top--3Af5JMX8")
sub_right
#%%
for item in sub_right:
    wsj_list.append(item.text)

sub_right = soup.find_all('div', class_='style--column--1p190TxH style--column-top--3Nm75EtS style--column-4--2Ng-GQLy')
sub_right
#%%
for item in sub_right:
    wsj_list.append(item.text)
...
payload = ""
headers = {
    "cookie": "wsjregion=na%252Cus; gdprApplies=false; ccpaApplies=false; vcdpaApplies=false; regulationApplies=gdpr%253Afalse%252Ccpra%253Afalse%252Cvcdpa%253Afalse; ab_uuid=859ac3e1-b43f-42f0-bda3-fc8b6dfca534",
    "User-Agent": "insomnia/8.4.2"
}
...
# class="css-11vqdys"
economy_list=[]
# div_content = soup.find_all('div', class_="css-11vqdys")
# for item in div_content:
    # economy_list.append(item.text)
# economy_list
div_content = soup.find_all('div', class_="css-11vqdys")
economy_list=div_content[0].text.split('min read')
economy_list


#%%
define='bulletpoint each point and summarize with this format. "[subject] has a good(or bad) news. Their [product/service] is...". However, if no specific subject like company, person, country is present, just summarize.'
economy_summary=gpt_answer(str(economy_list),define)
economy_summary
#%%
import requests
...
finance_list=[]
div_content = soup.find_all('div', class_="css-11vqdys")
finance_list=div_content[0].text.split('min read')
finance_list
#%%
define='bulletpoint each point and summarize with this format. "[subject] has a good(or bad) news. Their [product/service] is...". However, if no specific subject like company, person, country is present, just summarize.'
finance_summary=gpt_answer(str(finance_list),define)
finance_summary
#%%
...
search_list=save_images(wjslist)
...
search_list=save_images(economy_list)
make_inter_videos(search_list)
make_final_video(search_list,'wsj2')

search_list=save_images(finance_list)
make_inter_videos(search_list)
...
search_list=save_images(bio_list)
